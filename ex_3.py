...
total = 0
score_text = font.render(f"Score: {total}", True, WHITE)
...
while running:
    ...
    score_text = font.render(f"Score: {total}", True, WHITE)  # Обновляем счет
    ...
    if new_head[:2] == food[:2]:
        ...
        score_text = font.render(f"Score: {total}", True, WHITE)  # Обновляем счёт
    else:
        snake.pop()
